...
class SliceMesh:
    """2D coordinate mesh for a slice plane orthogonal to norm_dir, at norm_offset."""
...
class Mesh:
    """One FDS computational mesh block: 3D axis coordinates plus geometry helpers."""
...
def readMeshes(filename) -> "MeshCollection":
    """Scan an FDS .smv file for GRID/TRNX/TRNY/TRNZ records and build a MeshCollection."""
    import mmap

    mc = MeshCollection()

    logger.debug("scanning smv file for meshes: {}".format(filename))
    ...
    with mmap.mmap(infile.fileno(), 0, access=mmap.ACCESS_READ) as s:
        ...
        while cpos > 0:
            ...
            logger.debug("found MESH with label: {}".format(label))
            ...
            logger.debug("number of cells: {} x {} x {}".format( nx, ny, nz ))
...
class SliceCollection:
    """Ordered collection of Slice metadata objects parsed from one .smv file."""
...
class Slice:
    """One FDS slice-file (.sf) time series: metadata plus lazily-read data.

    index_ranges is [[x1,x2],[y1,y2],[z1,z2]] in mesh cell-index space; the
    axis where min==max is the slice's normal direction.
    """
    ...
    def __init__(self, quantity, label, units, filename, mesh_id, index_ranges, centered):
        ...
        if self.norm_direction is None:
            raise ValueError(
                "Slice '{}' ({}) is not axis-aligned on any of x/y/z (index_ranges={}); "
                "cannot determine its normal direction.".format(label, quantity, index_ranges))
    ...
    def readTimeSelection(self, root='.', dt=None, average_dt=None):
        ...
        if dt is None:
            logger.error("provide a dt for slice selection")
            return
...
#         print("slice index: ", slice_index)
        ...
        self.data_raw = np.zeros((nSlices, self.readSize), dtype=np.float32)
...
def combineSlices(slices):
    ...
    data = np.ones((slices[0].times.size, n2, n1), dtype=np.float32)
    mask = np.zeros((n2, n1))
    mask[:] = True
...
def readSliceInfos(filename):
    ...
    logger.debug("scanning smv file for slices: {}".format(filename))
    ...
    with mmap.mmap(infile.fileno(), 0, access=mmap.ACCESS_READ) as s:
        ...
        while cpos > 0:
            ...
            logger.debug("slice centered: {}".format(centered))
            ...
            logger.debug("slice info: {} {}".format(mesh_id, [[x1, x2], [y1, y2], [z1, z2]]))
...
def scanDirectory(directory: str):
    ...
    if len(list_fn_smv_abs) > 1:
        logger.warning("multiple smv files found, choosing an arbitrary file")

    return os.path.basename(list_fn_smv_abs[0])


def readSlice(root_dir, direction, offset, quantity, data_only=False):
    """Read all slices matching (quantity, direction, offset) from root_dir.

    Returns (mesh, extent, data, mask, times) by default, or just the
    combined data array if data_only is True.
    """
    ...
    slices = []
    for sid in sids:
        # read in time information, slice data, and map onto the mesh
        sid.readAllTimes(root_dir)
        sid.readData(root_dir)
        sid.mapData(meshes)
        slices.append(sid)

    if len(slices) == 0:
        logger.warning("no matching slices found for quantity=%s, direction=%s, offset=%s in %s",
...
def readDataOnly(root_dir, direction, offset, quantity):
    """Convenience wrapper around readSlice() that returns only the data array."""
    return readSlice(root_dir, direction, offset, quantity, data_only=True)
