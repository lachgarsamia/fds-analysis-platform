...
class TestProbeMeasurePanel:
    ...
    def test_wrapper_holds_devices_and_velocity_as_tabs(self, qapp):
        ...
        assert wrapper.tabs.widget(3) is window.lic_flow_panel
        assert wrapper.tabs.widget(4) is window.tracer_flow_panel
        assert not hasattr(window, "measurement_panel")
        assert not hasattr(window, "composite_flow_panel")
