...
| ✅ | M3.1: `analytics/features.py` (per-scenario feature vectors, deterministic, pure NumPy on cached arrays) + `analytics/clustering.py` (sklearn PCA/KMeans + `cluster_alignment()`) + `analytics_panel.py` (static PCA scatter dock, never connected to `TimeController`) + `auto_summary.py` (deterministic sentence templates from already-computed numbers + Markdown export). Real-data verification: 2-cluster KMeans aligns with candle count at 83.3% vs. 50.0% chance-level for door width (pinned regression test); all 24 scenarios' peak temperatures classify "near the candle", none "near the door", consistent with M2.3's finding; the vent-open-vs-closed comparison sentence goes the *opposite* direction from ROADMAP's own illustrative example (real data reported honestly either way) | `src/analytics/features.py`, `src/analytics/clustering.py`, `src/analytics_panel.py`, `src/auto_summary.py`, `src/browser.py`, `src/main_window.py`, `pyproject.toml` (new dep: scikit-learn), `tests/test_features.py`, `tests/test_clustering.py`, `tests/test_analytics_panel.py`, `tests/test_auto_summary.py`, `tests/test_integration.py`, merged to `main` |
...
main.py (bootstrap, splash)
   └─ main_window.py (MainWindow — view only)
        ...
        └─ browser.py (docked experiment browser QTableView — M2.5;
                       auto-summary label + Markdown export button — M3.1)
        └─ analytics_panel.py (docked static PCA/clustering scatter — M3.1)
        └─ simulation_controller.py (SimulationController + _Worker QThread)
             └─ data_provider.py (SimulationData / DataLoadError / demo fallback)
                  ...
                  └─ auto_summary.py (deterministic per-scenario sentence templates — M3.1)
                  └─ analytics/features.py, analytics/clustering.py (feature vectors,
                                        sklearn PCA/KMeans over cached arrays — M3.1)
...
| # | Milestone | Why | Impact | Effort | Risk | Priority |
|---|---|---|---|---|---|---|
| M3.1 | Ensemble analytics: features, PCA/clustering panel, auto-summaries | Certain-payoff research; interpretable; demo-safe | High | 4 d | Low | **High** — ✅ done |
...
### M3.1 — Ensemble analytics + auto-summaries (Track A) ✅ DONE
Merged to `main`. **Objective met:** certain-payoff research delivered — feature vectors, a clustering panel with a real (not assumed) factor-alignment signal, and auto-summaries shown in the browser and exportable for all 24, every number traceable to an existing computation.

| Task | Detail |
|---|---|
| 3.1.1 `analytics/features.py` ✅ | `compute_scenario_features()`: per-scenario feature vector (downsampled to `CURVE_POINTS=24` points so scenarios with slightly different frame counts stay directly comparable) — max-T curve, hot-area-fraction curve (threshold matches M2.6's isotherm hazard band, `config.ISOTHERM_LEVELS`), spatial-mean curve, time-to-100/300/600°C. `as_vector()` flattens for PCA/clustering, using a documented sentinel (`NEVER_CROSSED_SECONDS`) for thresholds never reached rather than NaN/zero. Pure NumPy on `ScenarioStore`'s already-cached arrays — no new disk cache layer (cheap enough not to need one). Deterministic (verified directly, not just assumed: identical input → identical output). Files: `analytics/features.py`. |
| 3.1.2 PCA + clustering panel ✅ | `analytics/clustering.py`: `run_pca`/`run_clustering` (sklearn, standardized features, `random_state=42` throughout) + `cluster_alignment()`, turning "clusters align with an interpretable factor" from an eyeballed scatter into an actual number. `analytics_panel.py`: `AnalyticsPanelDock`, a **static** matplotlib PCA(2) scatter (color=cluster, marker=candle count) built once at construction — never connected to `TimeController`, which is the literal mechanism behind the DoD's "doesn't degrade playback" claim. Hover/click via nearest-point lookup with a bounded pick radius (same "don't claim a point regardless of distance" principle as M2.6's cursor probe). Tabbed with the M2.5 experiment browser dock rather than stacked. Hierarchical dendrogram (marked optional in spec) not built — KMeans's own scatter already satisfies the DoD's sanity check. New dependency: `scikit-learn` (`pyproject.toml`), added because this task needed it, not ahead of time. |
| 3.1.3 Auto-summary ✅ | `auto_summary.py`: `generate_summary()` fills a fixed template from numbers already computed elsewhere — peak temp + time-of-peak and the 300°C crossing time from `summary_stats.ScenarioSummary` (M2.5), a "(near the candle)"/"(near the door)" spatial descriptor from a direct argmax + M2.6's extent lookup, and a cross-scenario vent-1 (VOD) comparison sentence from real group means among same-candle-count scenarios. `export_markdown()` writes the *same* generated text a user already saw in the browser, so the export can never drift from the on-screen version. Shown per scenario via a read-only label under the experiment browser's table (updates on row selection); "Export summaries (Markdown)…" button exports all 24 at once. |

**Real-data findings, verified before shipping rather than assumed (same practice as M2.3/M2.6):**
- **Clustering:** 2-way KMeans over the real 24-scenario feature space aligns with candle count at **83.3%**, vs. exactly **50.0%** (chance level) for door width — a real, meaningfully-above-chance signal, not a perfect separation, and not the arbitrary factor either. Pinned as a regression test.
- **Peak location:** all 24 scenarios' peak temperatures land in the "near the candle" zone, none near the door — consistent with M2.3's DifferenceView finding (candle/plume dominates temperature signal, not the doorway).
- **Vent comparison:** the real group-mean difference goes the *opposite* direction from ROADMAP's own illustrative example ("vent-open variants peaked 85°C **lower**") — in this dataset, open-vent variants actually peak slightly **higher** on average (more oxygen feeding the fire outweighing ventilation cooling). The module reports whichever direction the real numbers show, not an assumed one.
- **Bug caught during verification:** `str.capitalize()` was silently lowercasing the "HVAC" acronym to "hvac" in the comparison sentence — fixed before this shipped, not left in.

**Tests:** 11 (`test_features.py`) + 14 (`test_clustering.py`, incl. the pinned real-data candle-vs-door alignment) + 9 (`test_analytics_panel.py`) + 18 (`test_auto_summary.py`, incl. 2 real-data tests covering the DoD's "verified against browser stats for 3 scenarios" requirement directly) + 8 `MainWindow` integration tests (panel presence/absence, tabbed placement, click-to-load, browser summary display, export write + cancel paths, and a structural — not timing-based, so not CI-flaky — check that the panel has none of `PlotView`'s per-frame methods).

**DoD:** clusters align with at least one interpretable factor (✅ 83.3% vs. 50.0% chance level, pinned) · summaries verified against browser stats for 3 scenarios (✅ cases 0/5/15, real-data test) · panel doesn't degrade playback (✅ verified structurally — the panel is never connected to `TimeController`).
